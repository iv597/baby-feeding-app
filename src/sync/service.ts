...
import { ensureDeviceId, getHouseholdId, setHouseholdId, getLastSyncAt, setLastSyncAt, getLocalBabiesChangedSince, getLocalFeedsChangedSince, upsertBabyFromRemote, upsertFeedFromRemote } from '../db';
...
export async function createHousehold(): Promise<string> {
  ...
  await supabase.from('members').insert({ id: generateId('mem_'), householdId, deviceId });
...
export async function joinHousehold(householdId: string): Promise<void> {
  ...
  if (!supabase) throw new Error('Supabase not configured');
  const deviceId = await ensureDeviceId();
  const { data: hh, error } = await supabase.from('households').select('id').eq('id', householdId).maybeSingle();
...
export async function syncNow(): Promise<{ pushed: number; pulled: number }> {
  ...
  if (remoteBabies) {
    for (const b of remoteBabies as any[]) {
      await upsertBabyFromRemote(b.id, b.name, b.birthdate, b.updatedAt, b.deleted);
      pulled += 1;
    }
  }
  ...
  const { data: remoteFeeds } = await supabase
    .from('feed_entries')
    .select('id,babyId,type,createdAt,quantityMl,durationMin,side,foodName,foodAmountGrams,notes,updatedAt,deleted')
    .eq('householdId', householdId)
    .gt('updatedAt', since);
