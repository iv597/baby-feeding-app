...
export default function HistoryScreen() {
    ...
    return (
        <SafeAreaView style={styles.container} edges={["top", "left", "right"]}>
            <Text variant="titleLarge" style={{ marginBottom: 1 }}>
                Recent
            </Text>
            <FlatList
                data={data}
                keyExtractor={(item, idx) =>
                    item.id ? String(item.id) : `row-${idx}`
                }
                renderItem={({ item }) => (
                    <List.Item
                        title={item.type}
                        description={`${renderSubtitle(item)}  ·  ${format(
                ...
                ItemSeparatorComponent={() => (
                    <View style={{ height: 1, backgroundColor: "#b49cdbff" }} />
                )}
            />
        </SafeAreaView>
    );
